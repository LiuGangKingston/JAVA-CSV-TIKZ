...
// Specific calculation to generate CSV files
    ...
    startinxofsoidal = a * Math.cos(startingangleofsoidal*bigfile.Deg2Rad);
    startinyofsoidal = b * Math.sin(startingangleofsoidal*bigfile.Deg2Rad);
    c=-Math.sqrt(a*a-b*b);
    ...
    for(i=startingline; i<=totallines; i++) {
       ...
       dk=d*Math.cos(startingangle*bigfile.Deg2Rad);
       bigf=a*a - dk*dk;
       t=b*b*(dk+Math.sqrt(bigf+dk*dk))/bigf;
       x=t*Math.cos(startingangle*bigfile.Deg2Rad)-d;
       y=t*Math.sin(startingangle*bigfile.Deg2Rad);
       yprime=-b*b*x/(a*a*y);
       z = Math.sqrt((b*b*x)*(b*b*x) + (a*a*y)*(a*a*y));
       if(x < 0.0e0) {
          if(y < 0.0e0) {
             tangentangle = Math.asin(b*b*x /z) * bigfile.Rad2Deg; }
          else {
             tangentangle = 180.0e0 + Math.acos(a*a*y /z) * bigfile.Rad2Deg; }
       }
       else {
          if(y  < 0.0e0) {
             tangentangle = Math.acos(-a*a*y /z) * bigfile.Rad2Deg;}
          else {
             tangentangle = Math.acos(-a*a*y /z) * bigfile.Rad2Deg;}
       }
       ...
       try {
            (bigfile.GetFileForRow(i)).write(c + "," + d + "," + startingangle + "," + dk + "," + bigf + ","
                                             + t + "," + x + "," + y + "," + yprime + ","
                                             + tangentangle + "," + normalangle + "," + incidentangle + ","
                                             + reflectangle + "," + bigfile.PickTypicalColor(i) + "\n");
       } catch (IOException e) {
         System.out.println("An error occurred when closing the file setup.scalars.csv, then stopped.");
         e.printStackTrace();
         System.exit(-9);
